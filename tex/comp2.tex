...
\begin{document}
	...
	\section{Orientação a Objeto (\stmt{class})}
...
\end{lstlisting}
	...
	Crie uma classe \texttt{Fracao} que implemente as seguintes operações e métodos:
	\begin{enumerate}
		\item Um método que simplifique a fração.
		\item \texttt{+}, \texttt{-}, \texttt{*}, \texttt{**} e \texttt{/}.
		\item \texttt{-} (\spm{neg}) e \texttt{~} (\spm{invert}).
	...
	Sabendo isso, faça uma classe \texttt{Polinomio} que implemente:
	\begin{enumerate}
	...
	Bônus:
	\begin{enumerate}
		\item O método especial \spm{repr} que deve retornar uma \type{string} que represente o polinômio $2 + x + 3x^{2}$ na forma \texttt{"2x\^{}0 + 1x\^{}1 + 3x\^{}2"}, por exemplo.
...
Exception: Isso não é grupo!
	...
	\subsubsection{Interlúdio: \texttt{Beep}}
	\inter
...
class Musica(list):
	...
	\begin{enumerate}
		...
		\item A divisão \texttt{\slash} (\spm{div})
	\end{enumerate}
	...
	\problem[2]{O Triângulo de Sierpinsky}
	...
	\problem[3]{O Método de Monte Carlo}
	...
	O Desafio agora é pegar os resultados da Questão {\bf Musical} e construir um Piano Virtual com o \texttt{Tkinter}! Desenhe as teclas em um \texttt{Canvas} e use o método \texttt{Canvas.bind} para associar letras como \texttt{a,s,d,f,g,h,j} e \texttt{k} às teclas brancas e \texttt{w,e,t,y} e \texttt{u} às pretas, por exemplo.
