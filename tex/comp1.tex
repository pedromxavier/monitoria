...
\begin{document}
	...
	\section{Números (\type{int}, \type{float}, \type{complex})}
	
	\section{\textit{Strings} e Texto (\type{str})}
	
	\section{Condições (\stmt{if}, \stmt{elif}, \stmt{else})}
	
	\section{Listas e Tuplas (\type{list}, \type{tuple})}	
	
	\section{Laços (\stmt{for}, \stmt{while})}
	
	\section{Conjuntos e Dicionários (\type{set}, \type{dict})}
	
	\section{Arquivos (\stmt{with}, \type{open})}
