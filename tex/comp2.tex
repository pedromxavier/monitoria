...
class Musica(list):
	...
	\begin{enumerate}
		\item Crie um \texttt{Canvas}. \textbf{Dica:} uma vez criado o \texttt{Canvas}, posicione ele com o método \texttt{pack}, passando as opções \texttt{expand=\stmt{True}} e \texttt{fill=\str{both}} para que o \texttt{Cavas} ocupe toda a tela.
...
root.mainloop()
	...
	Um outro tópico interessante de se falar, ainda no escopo da \textbf{aritmética de ponto-flutuante} (\type{float}), é a presença de três números especiais no padrão \textbf{IEE 784}: \texttt{nan}, \texttt{inf} e \texttt{-inf}.\\
